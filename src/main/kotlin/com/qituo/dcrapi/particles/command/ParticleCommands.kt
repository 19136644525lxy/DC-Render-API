...
class ParticleVortexCommand(
    ...
    override fun apply(particle: ControlableParticle, tick: Int) {
        ...
        // 计算切向速度（垂直于到中心的方向）
        val tangent = Vec3(-toCenter.z, 0.0, toCenter.x).normalize()
        
        // 计算向心力
        val distance = toCenter.length()
        val radialForce = toCenter.normalize().scale(-strength / distance.coerceAtLeast(1.0))
        
        // 组合速度
        val newVelocity = particle.getVelocity()
            .add(tangent.scale(strength * 0.1))
            .add(radialForce)
            .add(0.0, height, 0.0)
...
class ParticleOrbitCommand(
    ...
    private val axis: Vec3 = Vec3(0.0, 1.0, 0.0)
) : ParticleCommand {
    ...
    override fun apply(particle: ControlableParticle, tick: Int) {
        ...
        particle.setPosition(Vec3(x, y, z))
    }
}
...
class ParticleAttractionCommand(
    ...
    override fun apply(particle: ControlableParticle, tick: Int) {
        ...
        val force = toTarget.normalize().scale(strength / distance.coerceAtLeast(1.0))
...
class ParticleDragCommand(
    ...
    override fun apply(particle: ControlableParticle, tick: Int) {
        val velocity = particle.getVelocity()
        particle.setVelocity(velocity.scale(drag))
    }
}
